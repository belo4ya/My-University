...
.gen.sale.level.distribution <- function(sale_level, data_in) {
  ...
  for (i in 1:length(data_in)) {
    data_out[i] <-
      tryhard.random(0, data_in[i])  # тут можно оптимизировать
  }
  ...
  while ((sum(data_out) <= max_sum - abs(step) * 2) ||
         (sum(data_out) >= max_sum + abs(step) * 2)) {
...
.generate.data.supply <-
  function(days, goods, path, min, max, headers) {
...
.generate.data.sale <-
  function(days, goods, sale_level, path, min, headers) {
    ...
    in_table <-
      read.table(paste0(path, "/in.txt"), header = T, sep = ",")
    ...
    if (sale_level[1] != -1) {
      ...
      for (i in 2:length(out_table)) {
        out_table[i] <-
          .gen.sale.level.distribution(sale_level[i - 1], in_table[, i])
      }
    } else {
...
generate.supply.sale <- function(path = ".",
                                 ...
                                 headers = T) {
  if (max < 1)
    stop("Error: invalid value - max < 1")
  if (min < 0)
    stop("Error: invalid value - min < 0")
  if ((max - min) < 1)
    stop("Error: invalid value - (max - min) < 1")
  days <- 7
  goods <- "молоко"
  sale_level <- F
...
generate.supply.sale.many <- function(path = ".",
                                      ...
                                      headers = T) {
  if (days < 1)
    stop("Error: invalid value - days < 1")
  if (max < 1)
    stop("Error: invalid value - max < 1")
  if (min < 0)
    stop("Error: invalid value - min < 0")
