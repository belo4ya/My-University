...
create.custom.table <- function(title, rows, cols, 
                                ex_rows = c(), ex_cols = c()) {
...
get.data.in <- function(path) {
  files <- dir(path)
  data_in <- read.tables(paste0(path, "/", files[grep("*in.txt", files)]))
...
get.data.out <- function(path) {
  files <- dir(path)
  data_out <- read.tables(paste0(path, "/", files[grep("*out.txt", files)]))
...
set.footer.param <- function(table, n_stores, n_goods) {
  ...
  table[n_stores + 1, n_goods + 2] <- sum(table[n_stores + 1, ][(1 + 1):(n_goods + 1)])
...
get.revenue.table <- function(data_in, data_out, stores, goods) {
  ...
  revenue_table <- create.custom.table(title, stores, goods,
  ...
  revenue_table <- set.footer.param(revenue_table, n_stores, n_goods)
...
get.profit.table <- function(data_in, data_out, stores, goods) {
  ...
  n_stores <- length(stores)
  n_goods <- length(goods)
  profit_table <- create.custom.table(title, stores, goods,
                                      c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"),
                                      c("ИТОГ"))
  
  for (i in 1:n_stores) {
    for (j in 1:n_goods) {
      profit_table[i, j + 1] <- (sum(data_out[[i]][j + 1]) * SALE_PRICE -             # Будем считать,
...
get.selling.table <- function(data_in, data_out, stores, goods) {
  ...
  n_stores <- length(stores)
  n_goods <- length(goods)
  selling_table <- create.custom.table(title, stores, goods,
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"),
                                       c("ИТОГ"))
  ...
  selling_table <- set.footer.param(selling_table, n_stores, n_goods)
  
  return(selling_table)
}


get.utilization.table <- function(data_in, data_out, stores, goods) {
  title = "СПИСАНИЕ"
  n_stores <- length(stores)
  n_goods <- length(goods)
  utilization_table <- create.custom.table(title, stores, goods,
                                           c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"),
                                           c("ИТОГ"))
  ...
  utilization_table <- set.footer.param(utilization_table, n_stores, n_goods)
  
  return(utilization_table)
}


get.profitability.table <- function(data_in, data_out, stores, goods) {
  title = "РЕНТАБЕЛЬНОСТЬ"
  n_stores <- length(stores)
  n_goods <- length(goods)
  profitability_table <- create.custom.table(title, stores, goods,
                                      c("СРЕДНЕЕ", "МЕДИАНА"),
                                      c("СРЕДНЕЕ", "МЕДИАНА"))
  
  for (i in 1:n_stores) {
    for (j in 1:n_goods) {
      profit <- (sum(data_out[[i]][j + 1]) * SALE_PRICE -            
                   sum(data_in[[i]][j + 1]) * PURCHASE_PRICE -       
                   (sum(data_in[[i]][j + 1]) -                       
                      sum(data_out[[i]][j + 1])) * UTILIZATION_PRICE)
      revenue <- sum(data_out[[i]][j + 1]) * SALE_PRICE
      profitability_table[i, j + 1] <- round(profit / revenue * 100, 1)
    }
  }
...
get.uniformity.sales.table <- function(data_in, data_out, stores, goods) {
  title = "РАВНОМЕРНОСТЬ ПРОДАЖ (SD)"
  n_stores <- length(stores)
  n_goods <- length(goods)
  uniformity_sales_table <- create.custom.table(title, stores, goods, 
                                                c("СРЕДНЕЕ", "МЕДИАНА"),
                                                c("СРЕДНЕЕ", "МЕДИАНА"))

  for (i in 1:n_stores) {
    for (j in 1:n_goods) {
      uniformity_sales_table[i, j + 1] <- round(sd(data_out[[i]][j + 1][, 1]), 1)
    }
  }
  ...
  for (i in 1:n_goods) {
    ...
    # МЕДИАНА
    uniformity_sales_table[n_stores + 2, i + 1] <- round(median(item))
  }
  
  for (i in 1:n_stores) {
    store <- as.numeric(uniformity_sales_table[i, ][(1 + 1):(n_goods + 1)])
    # СРЕДНЕЕ left
    uniformity_sales_table[i, n_goods + 1 + 1] <- round(mean(store))
    # МЕДИАНА left
    uniformity_sales_table[i, n_goods + 2 + 1] <- round(median(store))
  }
...
set.footer.store <- function(table, n_days, n_goods) {
  ...
  table[n_days + 1, n_goods + 2] <- sum(table[n_days + 1, ][(1 + 1):(n_goods + 1)])
...
get.revenue.table.by.store <- function(n, path_to_data) {
  ...
  revenue_table <- create.custom.table(title, paste("день", days), goods,
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"), 
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"))
...
get.profit.table.by.store <- function(n, path_to_data) {
  ...
  profit_table <- create.custom.table(title, paste("день", days), goods,
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"), 
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"))
  
  for (i in 1:n_days) {
    for (j in 1:n_goods) {
      profit_table[i, j + 1] <- (data_out[i, j + 1] * SALE_PRICE - 
...
get.selling.table.by.store <- function(n, path_to_data) {
  ...
  selling_table <- create.custom.table(title, paste("день", days), goods,
                                      c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"), 
                                      c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"))
...
get.utilization.table.by.store <- function(n, path_to_data) {
  ...
  utilization_table <- create.custom.table(title, paste("день", days), goods,
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"), 
                                       c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"))
  
  for (i in 1:n_days) {
    for (j in 1:n_goods) {
      utilization_table[i, j + 1] <- data_in[i, j + 1] - data_out[i, j + 1]
    }
  }
  
  utilization_table <- set.footer.store(utilization_table, n_days, n_goods)
...
get.profitability.table.by.store <- function(n, path_to_data) {
  ...
  profitability_table <- create.custom.table(title, paste("день", days), goods,
                                           c("СРЕДНЕЕ", "МЕДИАНА"), 
                                           c("СРЕДНЕЕ", "МЕДИАНА"))
  
  for (i in 1:n_days) {
    for (j in 1:n_goods) {
      profit <- (data_out[i, j + 1] * SALE_PRICE - 
                   data_in[i, j + 1] * PURCHASE_PRICE - 
                   (data_in[i, j + 1] - 
                      data_out[i, j + 1]) * UTILIZATION_PRICE)
...
params:
  ...
  "}
get.analysis.by.param <- function(path_to_data, path_to_save = path_to_data,
                                  filters = ALL, w = F) {
  tables <- list()
  filters <- toupper(filters)
  if (filters[1] == ALL) {
    filters[1:6] <- c(REVENUE, PROFIT, SELLING, UTILIZATION, PROFITABILITY, UNIFORMITY)
  }
  ...
  if (REVENUE %in% filters) {
    tables[REVENUE] <- list(get.revenue.table(data_in, data_out, stores, goods))
  }
  
  if (PROFIT %in% filters) {
    tables[PROFIT] <- list(get.profit.table(data_in, data_out, stores, goods))
  }
  
  if (SELLING %in% filters) {
    tables[SELLING] <- list(get.selling.table(data_in, data_out, stores, goods))
  }
  
  if (UTILIZATION %in% filters) {
    tables[UTILIZATION] <- list(get.utilization.table(data_in, data_out, stores, goods))
  }
  
  if (PROFITABILITY %in% filters) {
    tables[PROFITABILITY] <- list(get.profitability.table(data_in, data_out, stores, goods))
  }
  
  if (UNIFORMITY %in% filters) {
    tables[UNIFORMITY] <- list(get.uniformity.sales.table(data_in, data_out, stores, goods))
  }
  ...
  for (i in 1:length(tables)) {
    write.csv2(tables[[filters[i]]], paste0(path_to_save, "/", tolower(filters[i]),".csv"), row.names = F)
  }
...
params:
  ...
  Если w = FALSE - возвращает, полученные таблицы.
  "}
get.analysis.by.store <- function(n, path_to_data, path_to_save = path_to_data,
                                  filters = ALL, w = F) {
  tables <- list()
  filters <- toupper(filters)
  if (filters[1] == ALL) {
    filters[1:5] <- c(REVENUE, PROFIT, SELLING, UTILIZATION, PROFITABILITY)
  }
  ...
  if (UTILIZATION %in% filters) {
    tables[UTILIZATION] <- list(get.utilization.table.by.store(n, path_to_data))
  }
  
  if (PROFITABILITY %in% filters) {
    tables[PROFITABILITY] <- list(get.profitability.table.by.store(n, path_to_data))
  }
