clear.table <-
  function(table,
           del_row = c("ИТОГ", "СРЕДНЕЕ", "МЕДИАНА"),
           del_col = del_row) {
    return(table[!(table[, 1] %in% del_row),!(colnames(table) %in% del_col)])
  }


# ========================== ПО ОДНОМУ МАГАЗИНУ ========================== #

plot.trend.lines.by.store <-
  function(data_table,
           path_to_save = "./",
           goods = NULL,
           v = F) {
    ...
    data_table <-
      clear.table(data_table, del_col = colnames(data_table)[1])
    
    if (is.null(goods))
      goods <- colnames(data_table)
...
plot.trend.lines.by.product <-
  function(data_tables,
    ...
    png(paste0(
      ...
      tolower(product),
      "_",
      paste0(legend_params, collapse = "_"),
      ".png"
    ),
    pointsize = 15)
...
plot.scatter.plot.by.params <- function(data_tables,
                                        product,
...
plot.store.sum <- function(store, path_to_save = "./") {
  for (i in c(REVENUE, PROFIT, SELLING, UTILIZATION)) {
    ...
    if (i %in% c(SELLING, UTILIZATION)) {
      axis(
        2,
        at = seq(ifelse(min(results) < 0, min(results), 0), max(results), by = 100),
        labels = seq(ifelse(min(results) < 0, min(results), 0), max(results), by = 100)
      )
    } else {
      axis(
        2,
        at = seq(ifelse(min(results) < 0, min(results), 0), max(results), by = 10000),
...
plot.selling.stores.goods <-
  function(stores,
    ...
    m <-
      matrix(nrow = length(stores_names),
             ncol = length(goods_names))
...
plot.selling.trend.stores <- function(stores, path_to_save = "./") {
  days <- 1:length(clear.table(stores[[1]]$SELLING)[1][, 1])
  xrange <- range(days)
  yrange <-
    c(min(clear.table(stores[[1]]$SELLING)[2:length(clear.table(stores[[1]]$SELLING))]),
      max(clear.table(stores[[1]]$SELLING)[2:length(clear.table(stores[[1]]$SELLING))]))
  for (i in stores) {
    mn <- min(clear.table(i$SELLING)[2:length(clear.table(i$SELLING))])
    mx <-
      max(clear.table(i$SELLING)[2:length(clear.table(i$SELLING))])
    yrange[1] <- ifelse(mn < yrange[1], mn, yrange[1])
    yrange[2] <- ifelse(mx > yrange[2], mx, yrange[2])
  }
  
  main <- unlist(strsplit(colnames(stores[[1]]$SELLING)[1], " "))
  main <- toupper(main[length(main)])
  
  png(
    paste0(path_to_save, "./all_stores_all_goods.png"),
    width = 1000,
    height = 1200
  )
  par(mar = c(5, 5, 5, 15))
  ...
  for (i in 1:length(stores)) {
    store <- clear.table(stores[[i]]$SELLING)
    for (j in 2:length(store)) {
      points(c(1:30),
             store[j][, 1],
             pch = j + 14,
             col = i)
  ...
  par(xpd = TRUE)
  goods <-
    colnames(clear.table(stores[[1]]$SELLING))[2:length(clear.table(stores[[1]]$SELLING))]
  legend(
    x = "topleft",
    legend = goods,
    inset = c(1.05, 0.25),
    title = "Товары",
    pch = c(1:length(goods) + 14),
    cex = 2,
    col = 1,
  )
  stores <- paste("Магазин", 1:length(stores))
  legend(
    x = "bottomleft",
    legend = stores,
    inset = c(1.05, 0.25),
